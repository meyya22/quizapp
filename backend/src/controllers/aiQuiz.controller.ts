...
export async function expandAiQuiz(req: AuthRequest, res: Response): Promise<void> {
  const userId = req.user!.id;
  const { id } = req.params;
  ...
  const planKey = getPlanKey(userRecord!.tier, userRecord!.subscriptionPlan);
  if (planKey !== 'EXAMELITE') {
    res.status(403).json({ error: 'Expanding quizzes is available on the ExamElite plan only.' });
    return;
  }
  ...
  const existingQuestions = quiz.questions as unknown as Array<Record<string, unknown>>;
  const currentCount = existingQuestions.length;

  if (currentCount >= MAX_QUIZ_QUESTIONS) {
    res.status(400).json({ error: `This quiz has reached the maximum of ${MAX_QUIZ_QUESTIONS} questions.` });
    return;
  }

  const toGenerate = Math.min(EXPAND_BATCH, MAX_QUIZ_QUESTIONS - currentCount);
...
- "explanation": one or two sentence explanation of why the answer is correct`;
  ...
  const newQuestions = rawQuestions
    ...
    .filter((q): q is Record<string, unknown> => {
      if (!q || typeof q !== 'object') return false;
      const qo = q as Record<string, unknown>;
      return typeof qo.text === 'string' && validTypes.includes(qo.type as string) && qo.correctAnswer !== undefined;
    })
    .map((q, idx) => ({
      id: `q_${currentCount + idx}`,
      type: q.type as string,
      text: String(q.text).slice(0, 1000),
      options: (q.type === 'MULTIPLE_CHOICE' || q.type === 'MULTIPLE_RESPONSE') && q.options ? q.options : null,
      correctAnswer: q.correctAnswer,
      explanation: q.explanation ? String(q.explanation).slice(0, 1000) : '',
    }));

  if (newQuestions.length === 0) {
    res.status(502).json({ error: 'AI did not generate valid questions. Please try again.' });
    return;
  }

  const mergedQuestions = [...existingQuestions, ...newQuestions];

  const updated = await prisma.aiQuiz.update({
    where: { id },
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    data: { questions: JSON.parse(JSON.stringify(mergedQuestions)) as any, numQuestions: mergedQuestions.length },
  });
